...
\alignauthor \normalsize{Laurin Paech,  Sven Knobloch, David Yenicelik}\\
	\affaddr{\normalsize{knsven 14-945-166, lpaech 15-944-242, winbergs - 15-941-222,  cfriess  15-943-111, isaakh 15-913-312, yedavid 15-944-366}}\\
	\email{\normalsize{one@student.ethz.ch, two@student.ethz.ch, three@student.ethz.ch, one@student.ethz.ch, two@student.ethz.ch, three@student.ethz.ch}}
}
...
\begin{abstract}
We propose a novel social networking application based on photos. 
A person can take a picture, which is then shared amongst all physically close phones that are part of the network. \\
Due to it's architecture, the Android app does not rely on high-bandwidth connections from a centralized server, but rather uses a distributed network peer to peer network to transmit photos between individual nodes.\\
Out proposed app uses a central tracker to inform each participating node about all other participating nodes in the network. 
Then, nodes can transmit pictures within the network. 
These transmissions are tracked through a centralized Amazon AWS server which helps coordinate transmissions if photos are not yet sent or received. 
Using a peer to peer transmission protocol, small chunks of the picture file are transmitted from different nodes in the network. 
As such, this tracker acts as a coordinator, but does not rely on a high-bandwith or a high-load connection. \\
Our goal is to have a fully-working prototype with the proposed architecture, where a single node delivers to another node, and where a node can ping other nodes if it is 'missing' a photo. 
